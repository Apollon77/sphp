...
sphp.setOptions = function(option, callback){
  ...
  if(typeof option.cgiEngine === 'string'){
    ...
    child.on('close', function () {
      var version = child.resp.split('\n')[0];
      if(version.length <1)
        throw new Error('PHP engine failed to start (' + sphp.cgiEngine +'): Invalid Response: ' + version);
      process.versions.php = version;
    ...
    child.on('error', (error) => {
      throw new Error('PHP engine failed to start (' + sphp.cgiEngine +'): ' + error);
      if( typeof callback === 'function') {
        callback();
        callback = null;
      }
